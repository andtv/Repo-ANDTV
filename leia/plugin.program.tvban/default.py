	...
	if not WORKINGURL == True:
		...
		if total == 1 and third == False:
			for name, version, url, gui, kodi, theme, icon, fanart, adult, description in match:
				...
				return
		addFile('%s Version: %s' % (MCNAME, KODIV), '', icon=ICONBUILDS, themeit=THEME3)
		addFile('[COLOR gold]Version TVBAN Instalada:[/COLOR] %s %s' % (BUILDNAME, BUILDVERSION))              
		addDir ('Conservar Datos Importantes'       ,'savedata', icon=ICONSAVE,     themeit=THEME3)       
		if HIDESPACERS == 'No': addFile(wiz.sep(), '', themeit=THEME3)
		if third == True:
			for item in addin:
		...
		if len(match) >= 1:
			if SEPERATE == 'true':
				...
				if count18 > 0:
					...
					addFile('[B]%s Builds TVBAN (%s)[/B]' % (state, count18), 'togglesetting',  'show18', themeit=THEME3)
					if SHOW18 == 'false':
						for name, version, url, gui, kodi, theme, icon, fanart, adult, description in match:
							if not SHOWADULT == 'true' and adult.lower() == 'yes': continue
							if not DEVELOPER == 'true' and wiz.strTest(name): continue
				...
				if count16 > 0:
					...
					addFile('[B]%s [COLOR dodgerblue]Parches TVBAN (%s)[/COLOR][/B]' % (state, count16), 'togglesetting',  'show16', themeit=THEME3)
					if SHOW16 == 'false':
						for name, version, url, gui, kodi, theme, icon, fanart, adult, description in match:
							if not SHOWADULT == 'true' and adult.lower() == 'yes': continue
							if not DEVELOPER == 'true' and wiz.strTest(name): continue
				...
				if count15 > 0:
					state = '[COLOR orange]+[/COLOR]' if SHOW15 == 'false' else '[COLOR orange]-[/COLOR]'
					addFile('[B]%s [COLOR orange]Instalar MOTOR (%s)[/COLOR][/B]' % (state, count15), 'togglesetting',  'show15', themeit=THEME2)
					if SHOW15 == 'false':
						for name, version, url, gui, kodi, theme, icon, fanart, adult, description in match:
							if not SHOWADULT == 'true' and adult.lower() == 'yes': continue
							if not DEVELOPER == 'true' and wiz.strTest(name): continue
				...
				if count17 > 0:
					...
					addFile('[B]%s Gracias por Instalar TVBAN (%s)[/B]' % (state, count17), 'togglesetting',  'show17', themeit=THEME3)
					if SHOW17 == 'false':
						for name, version, url, gui, kodi, theme, icon, fanart, adult, description in match:
							if not SHOWADULT == 'true' and adult.lower() == 'yes': continue
							if not DEVELOPER == 'true' and wiz.strTest(name): continue
...
def viewBuild(name):
    ...
    for version, url, gui, kodi, themefile, icon, fanart, preview, adult, info, description in match:
        ...
        if warning == True:
            addFile('[COLOR white][B]INSTALAR:[/B][/COLOR] [COLOR orange][B]MOTOR[/B][/COLOR] / [COLOR dodgerblue][B]PARCHE[/B][/COLOR]', 'install', name, 'normal' , description=description, fanart=fanart, icon=icon, themeit=THEME1)
                    ...
                    if plugin.lower() == 'section':
                        ...
                        try:
                            ...
                            if os.path.exists(add):
                                aname   = "[COLOR dodgerblue][Instalado][/COLOR] %s" % aname
                        except:
                            pass
                        x += 1
                ...
                for name, url, repository, repositoryxml, repositoryurl, icon, fanart, adult, description in match:
                    if os.path.exists(os.path.join(ADDONS, plugin)):
                        ...
                        if selected == 0:
                            ...
                            if DIALOG.yesno(ADDONTITLE, "[COLOR %s]Quieres borrar la carpeta addon_data de:" % COLOR2, "[COLOR %s]%s[/COLOR]?[/COLOR]" % (COLOR1, plugin), yeslabel="[B][COLOR dodgerblue]Borrar[/COLOR][/B]", nolabel="[B][COLOR red]Saltar[/COLOR][/B]"):
                    ...
                    if not repository.lower() == 'none' and not os.path.exists(repo):
                        ...
                        if DIALOG.yesno(ADDONTITLE, "[COLOR %s]Would you like to install the repository for [COLOR %s]%s[/COLOR]:" % (COLOR2, COLOR1, plugin), "[COLOR %s]%s[/COLOR]?[/COLOR]" % (COLOR1, repository), yeslabel="[B][COLOR dodgerblue]Yes Install[/COLOR][/B]", nolabel="[B][COLOR red]No Skip[/COLOR][/B]"):
...
def maintMenu(view=None):
    on = '[B][COLOR dodgerblue]ON[/COLOR][/B]'; off = '[B][COLOR red]OFF[/COLOR][/B]'
    ...
    wizlogsize = ': [COLOR red]No hay[/COLOR]' if not os.path.exists(WIZLOG) else ": [COLOR dodgerblue]%s[/COLOR]" % wiz.convertSize(os.path.getsize(WIZLOG))
    ...
    if view == "clean" or SHOWMAINT == 'true':
        addFile('Limpieza total: [COLOR dodgerblue][B]%s[/B][/COLOR]' % wiz.convertSize(totalsize),    'fullclean',       icon=ICONMAINT, themeit=THEME3)
        addFile('Limpiar Cache: [COLOR dodgerblue][B]%s[/B][/COLOR]' % wiz.convertSize(sizecache),       'clearcache',      icon=ICONMAINT, themeit=THEME3)
        addFile('Limpiar paquetes: [COLOR dodgerblue][B]%s[/B][/COLOR]' % wiz.convertSize(sizepack),     'clearpackages',   icon=ICONMAINT, themeit=THEME3)
        addFile('Limpiar imagenes: [COLOR dodgerblue][B]%s[/B][/COLOR]' % wiz.convertSize(sizethumb),  'clearthumb',      icon=ICONMAINT, themeit=THEME3)
        if os.path.exists(ARCHIVE_CACHE): addFile('Limpiar Cache de Archivos: [COLOR dodgerblue][B]%s[/B][/COLOR]' % wiz.convertSize(archive), 'cleararchive',    icon=ICONMAINT, themeit=THEME3)
    ...
    if view == "misc" or SHOWMAINT == 'true':
        ...
        addDir ('Forzar cierre de Kodi',               'forceclose',      icon=ICONMAINT, themeit=THEME3)
    ...
    if view == "autoclean" or SHOWMAINT == 'true':
        addFile('--- Frecuencia de Auto Limpieza: [B][COLOR dodgerblue]%s[/COLOR][/B]' % feq[AUTOFEQ], 'changefeq', icon=ICONMAINT, themeit=THEME3)
	...
	addDir ('[I]<< Volver al Menu Principal TVBAN[/I]', icon=ICONMAINT, themeit=THEME2)	    
...
def writeAdvanced(name, url):
    ...
    if ADVANCEDWORKING == True:
        if os.path.exists(ADVANCED): choice = DIALOG.yesno(ADDONTITLE, "[COLOR %s]Quieres sobreescribir el actual Buffer con [COLOR %s]%s[/COLOR]?[/COLOR]" % (COLOR2, COLOR1, name), yeslabel="[B][COLOR dodgerblue]Si[/COLOR][/B]", nolabel="[B][COLOR red]No[/COLOR][/B]")
        else: choice = DIALOG.yesno(ADDONTITLE, "[COLOR %s]Quieres descargarlo e instalarlo [COLOR %s]%s[/COLOR]?[/COLOR]" % (COLOR2, COLOR1, name), yeslabel="[B][COLOR dodgerblue]Instalar[/COLOR][/B]", nolabel="[B][COLOR red]Cancelar[/COLOR][/B]")
...
def saveMenu():
    on = '[COLOR dodgerblue]ON[/COLOR]'; off = '[COLOR red]OFF[/COLOR]'
...
def traktMenu():
    trakt = '[COLOR dodgerblue]ON[/COLOR]' if KEEPTRAKT == 'true' else '[COLOR red]OFF[/COLOR]'
    ...
    for trakt in traktit.ORDER:
        if xbmc.getCondVisibility('System.HasAddon(%s)' % TRAKTID[trakt]['plugin']):
            ...
            else:                        addFile('[COLOR dodgerblue]Addon Data: %s[/COLOR]' % auser,'authtrakt', trakt, icon=icon, fanart=fanart, menu=menu)
            if user == "":
            ...
            else:                        addFile('[COLOR dodgerblue]Saved Data: %s[/COLOR]' % user, '', icon=icon, fanart=fanart, menu=menu2)
...
def realMenu():
    real = '[COLOR dodgerblue]ON[/COLOR]' if KEEPREAL == 'true' else '[COLOR red]OFF[/COLOR]'
    ...
    for debrid in debridit.ORDER:
        if xbmc.getCondVisibility('System.HasAddon(%s)' % DEBRIDID[debrid]['plugin']):
            ...
            else:                        addFile('[COLOR dodgerblue]Addon Data: %s[/COLOR]' % auser,'authdebrid', debrid, icon=icon, fanart=fanart, menu=menu)
            if user == "":
...
def loginMenu():
    login = '[COLOR dodgerblue]ON[/COLOR]' if KEEPLOGIN == 'true' else '[COLOR red]OFF[/COLOR]'
    ...
    for login in loginit.ORDER:
        if xbmc.getCondVisibility('System.HasAddon(%s)' % LOGINID[login]['plugin']):
            ...
            else:                        addFile('[COLOR dodgerblue]Addon Data: %s[/COLOR]' % auser,'authlogin', login, icon=icon, fanart=fanart, menu=menu)
            if user == "":
...
def removeAddonMenu():
    ...
    if len(selected) > 0:
        ...
        else: todo = DIALOG.yesno(ADDONTITLE, "[COLOR %s]Quieres [COLOR %s]Forzar el cierre[/COLOR]de Kodi o [COLOR %s]Recargar el perfil[/COLOR]? La Opcion recomendada es CERRAR.[/COLOR]" % (COLOR2, COLOR1, COLOR1), yeslabel="[B][COLOR red]Recargar[/COLOR][/B]", nolabel="[B][COLOR dodgerblue]Cerrar Kodi[/COLOR][/B]")
...
def removeAddonDataMenu():
    if os.path.exists(ADDOND):
        ...
        for folder in sorted(fold, key = lambda x: x):
            ...
            replace = {'audio.':'[COLOR orange][AUDIO] [/COLOR]', 'metadata.':'[COLOR cyan][METADATA] [/COLOR]', 'module.':'[COLOR orange][MODULE] [/COLOR]', 'plugin.':'[COLOR teal][PLUGIN] [/COLOR]', 'program.':'[COLOR orange][PROGRAM] [/COLOR]', 'repository.':'[COLOR gold][REPO] [/COLOR]', 'script.':'[COLOR dodgerblue][SCRIPT] [/COLOR]', 'service.':'[COLOR dodgerblue][SERVICE] [/COLOR]', 'skin.':'[COLOR dodgerblue][SKIN] [/COLOR]', 'video.':'[COLOR orange][VIDEO] [/COLOR]', 'weather.':'[COLOR yellow][WEATHER] [/COLOR]'}
            for rep in replace:
                folderdisplay = folderdisplay.replace(rep, replace[rep])
            if foldername in EXCLUDES: folderdisplay = '[COLOR dodgerblue][B][BLOQUEADO][/B][/COLOR] %s' % folderdisplay
    ...
    for folder in sorted(fold, key = lambda x: x):
        ...
        if os.path.exists(addonxml):
            ...
            try:
                ...
                state  = "[COLOR dodgerblue][Activado][/COLOR]"
                goto   = "false"
            except:
...
def buildWizard(name, type, theme=None, over=False):
    ...
    if type == 'gui':
        if name == BUILDNAME:
            if over == True: yes = 1
            else: yes = DIALOG.yesno(ADDONTITLE, '[COLOR %s]Would you like to apply the guifix for:' % COLOR2, '[COLOR %s]%s[/COLOR]?[/COLOR]' % (COLOR1, name), nolabel='[B][COLOR red]No, Cancel[/COLOR][/B]',yeslabel='[B][COLOR dodgerblue]Apply Fix[/COLOR][/B]')
        else:
            yes = DIALOG.yesno("%s - [COLOR red]WARNING!![/COLOR]" % ADDONTITLE, "[COLOR %s][COLOR %s]%s[/COLOR] community build is not currently installed." % (COLOR2, COLOR1, name), "Would you like to apply the guiFix anyways?.[/COLOR]", nolabel='[B][COLOR red]No, Cancel[/COLOR][/B]',yeslabel='[B][COLOR dodgerblue]Apply Fix[/COLOR][/B]')
        if yes:
            ...
            else: todo = DIALOG.yesno(ADDONTITLE, "[COLOR %s]The Gui fix has been installed.  Quieres Forzar el cierre de Kodi o Recargar el perfil?[/COLOR]" % COLOR2, yeslabel="[B][COLOR red]Recargar[/COLOR][/B]", nolabel="[B][COLOR dodgerblue]Cerrar Kodi[/COLOR][/B]")
        ...
        if warning == True:
            yes_pressed = DIALOG.yesno("%s - [COLOR red]ATENCION[/COLOR]" % ADDONTITLE, '[COLOR %s]Vas a instalar un Parche para la BUILD AURA-TVBAN' % COLOR2, 'Quieres instalar: [COLOR %s]%s v%s[/COLOR]?[/COLOR]' % (COLOR1, name, wiz.checkBuild(name,'version')), nolabel='[B][COLOR red]Cancelar[/COLOR][/B]',yeslabel='[B][COLOR dodgerblue]Instalar[/COLOR][/B]')
        else:
            if not over == False: yes_pressed = 1
            else: yes_pressed = DIALOG.yesno(ADDONTITLE, '[COLOR %s]Estas a punto de instalar el [COLOR gold][B]Parche[/B][/COLOR]' % COLOR2, '[COLOR %s]%s v%s[/COLOR]?[/COLOR]' % (COLOR1, name, wiz.checkBuild(name,'version')), nolabel='[B][COLOR red]Cancelar[/COLOR][/B]',yeslabel='[B][COLOR dodgerblue]Instalar[/COLOR][/B]')
        if yes_pressed:
            wiz.clearS('build')
            ...
            if int(float(percent)) > 0:
                ...
                if int(float(errors)) > 0:
                    yes=DIALOG.yesno(ADDONTITLE, '[COLOR %s][COLOR %s]%s v%s[/COLOR]' % (COLOR2, COLOR1, name, wiz.checkBuild(name,'version')), 'Completado: [COLOR %s]%s%s[/COLOR] [Errores:[COLOR %s]%s[/COLOR]]' % (COLOR1, percent, '%', COLOR1, errors), 'Quieres ver los errores?[/COLOR]', nolabel='[B][COLOR red]No[/COLOR][/B]', yeslabel='[B][COLOR dodgerblue]Si[/COLOR][/B]')
                    if yes:
                ...
                else: todo = DIALOG.yesno(ADDONTITLE, "[COLOR %s]Quieres [COLOR %s]CERRAR[/COLOR] Kodi o [COLOR %s]Recargar el perfil[/COLOR]? La opcion recomendada es CERRAR[/COLOR]" % (COLOR2, COLOR1, COLOR1), yeslabel="[B][COLOR red]Recargar[/COLOR][/B]", nolabel="[B][COLOR dodgerblue]Cerrar[/COLOR][/B]")
                if todo == 1: wiz.reloadFix()
        ...
        if theme == None:
            ...
            if not themefile == 'http://' and wiz.workingURL(themefile) == True:
                ...
                if len(themelist) > 0:
                    if DIALOG.yesno(ADDONTITLE, "[COLOR %s]The Build [COLOR %s]%s[/COLOR] comes with [COLOR %s]%s[/COLOR] different themes" % (COLOR2, COLOR1, name, COLOR1, len(themelist)), "Would you like to install one now?[/COLOR]", yeslabel="[B][COLOR dodgerblue]Install Theme[/COLOR][/B]", nolabel="[B][COLOR red]Cancel Themes[/COLOR][/B]"):
        ...
        else: installtheme = DIALOG.yesno(ADDONTITLE, '[COLOR %s]Would you like to install the theme:' % COLOR2, '[COLOR %s]%s[/COLOR]' % (COLOR1, theme), 'for [COLOR %s]%s v%s[/COLOR]?[/COLOR]' % (COLOR1, name, wiz.checkBuild(name,'version')), yeslabel="[B][COLOR dodgerblue]Install Theme[/COLOR][/B]", nolabel="[B][COLOR red]Cancel Themes[/COLOR][/B]")
        if installtheme:
...
def thirdPartyInstall(name, url):
    ...
    type = DIALOG.yesno(ADDONTITLE, "[COLOR %s]Instalacion [COLOR %s]Con Limpieza (Builds)[/COLOR] o [COLOR %s]Sin Limpieza (Parches)[/COLOR] Para el paquete:[/COLOR]" % (COLOR2, COLOR1, COLOR1), "[COLOR %s]%s[/COLOR]" % (COLOR1, name), yeslabel="[B][COLOR dodgerblue]Con Limpieza[/COLOR][/B]", nolabel="[B][COLOR red]Sin Limpieza[/COLOR][/B]")
    if type == 1:
    ...
    if int(float(percent)) > 0:
        ...
        if int(float(errors)) > 0:
            yes=DIALOG.yesno(ADDONTITLE, '[COLOR %s][COLOR %s]%s[/COLOR]' % (COLOR2, COLOR1, name), 'Completedado: [COLOR %s]%s%s[/COLOR] [Errores:[COLOR %s]%s[/COLOR]]' % (COLOR1, percent, '%', COLOR1, errors), 'Quieres ver los errores?[/COLOR]', nolabel='[B][COLOR red]No[/COLOR][/B]',yeslabel='[B][COLOR dodgerblue]Ver Errores[/COLOR][/B]')
            if yes:
                if isinstance(errors, unicode):
                    error = error.encode('utf-8')
                wiz.TextBox(ADDONTITLE, error)
    ...
    else: todo = DIALOG.yesno(ADDONTITLE, "[COLOR %s]Quieres [COLOR %s]Forzar el cierre de[/COLOR] Kodi o [COLOR %s]Recargar el perfil[/COLOR]?[/COLOR]" % (COLOR2, COLOR1, COLOR1), yeslabel="[B][COLOR red]Recargar perfil[/COLOR][/B]", nolabel="[B][COLOR dodgerblue]Cerrar Kodi[/COLOR][/B]")
    if todo == 1: wiz.reloadFix()
    else: wiz.killxbmc(True)
...
def apkInstaller(apk, url):
    ...
    if wiz.platform() == 'android':
        yes = DIALOG.yesno(ADDONTITLE, "[COLOR %s]Quieres descargar e instalar:" % COLOR2, "[COLOR %s]%s[/COLOR]" % (COLOR1, apk), yeslabel="[B][COLOR dodgerblue]Descargar[/COLOR][/B]", nolabel="[B][COLOR red]Cancelar[/COLOR][/B]")
...
def romInstaller(name, url):
    ...
    if myroms == '':
        if DIALOG.yesno(ADDONTITLE, "[COLOR %s]It seems that you do not have an extract location setup for Rom Packs" % COLOR2, "Would you like to set one?[/COLOR]", yeslabel="[COLOR dodgerblue][B]Set Location[/B][/COLOR]", nolabel="[COLOR red][B]Cancel Download[/B][/COLOR]"):
            wiz.openS()
            myroms = wiz.getS('rompath')
            if myroms == '': return
    yes = DIALOG.yesno(ADDONTITLE, "[COLOR %s]Are you sure you would like to download and extract [COLOR %s]%s[/COLOR] to:" % (COLOR2, COLOR1, name), "[COLOR %s]%s[/COLOR]" % (COLOR1, myroms), yeslabel="[B][COLOR dodgerblue]Download[/COLOR][/B]", nolabel="[B][COLOR red]Cancel[/COLOR][/B]")
...
def removeAddon(addon, name, over=False):
    if not over == False:
        ...
        yes = DIALOG.yesno(ADDONTITLE, '[COLOR %s]Are you sure you want to delete the addon:'% COLOR2, 'Name: [COLOR %s]%s[/COLOR]' % (COLOR1, name), 'ID: [COLOR %s]%s[/COLOR][/COLOR]' % (COLOR1, addon), yeslabel='[B][COLOR dodgerblue]Remove Addon[/COLOR][/B]', nolabel='[B][COLOR red]Don\'t Remove[/COLOR][/B]')
    if yes == 1:
...
def removeAddonData(addon, name=None, over=False):
    if addon == 'all':
        if DIALOG.yesno(ADDONTITLE, '[COLOR %s]Quieres eliminar [COLOR %s]TODOS[/COLOR] los datos de la carpeta Addon_data?[/COLOR]' % (COLOR2, COLOR1), yeslabel='[B][COLOR dodgerblue]Eliminar Data[/COLOR][/B]', nolabel='[B][COLOR red]No Eliminar[/COLOR][/B]'):
        ...
        if DIALOG.yesno(ADDONTITLE, '[COLOR %s]Quieres eliminar [COLOR %s]TODOS[/COLOR] los datos de la carpeta Addon_data de addons desinstalados?[/COLOR]' % (COLOR2, COLOR1), yeslabel='[B][COLOR dodgerblue]Eliminar Data[/COLOR][/B]', nolabel='[B][COLOR red]No Eliminar[/COLOR][/B]'):
            total = 0
        ...
        if DIALOG.yesno(ADDONTITLE, '[COLOR %s]Quieres eliminar [COLOR %s]TODAS[/COLOR] las carpetas vacias de userdata?[/COLOR]' % (COLOR2, COLOR1), yeslabel='[B][COLOR dodgerblue]Borrar Data[/COLOR][/B]', nolabel='[B][COLOR red]No Eliminar[/COLOR][/B]'):
        ...
        if addon in EXCLUDES:
            ...
            if DIALOG.yesno(ADDONTITLE, '[COLOR %s]Quieres borrar de addon_data:[/COLOR]' % COLOR2, '[COLOR %s]%s[/COLOR]' % (COLOR1, addon), yeslabel='[B][COLOR dodgerblue]Eliminar Data[/COLOR][/B]', nolabel='[B][COLOR red]No eliminar[/COLOR][/B]'):
...
def manageSaveData(do):
    if do == 'import':
        ...
        x = 0
        overwrite = DIALOG.yesno(ADDONTITLE, "[COLOR %s]Would you rather we overwrite all Save Data files or ask you for each file being imported?[/COLOR]" % (COLOR2), yeslabel="[B][COLOR dodgerblue]Overwrite All[/COLOR][/B]", nolabel="[B][COLOR red]No Ask[/COLOR][/B]")
            ...
            for item in files:
                ...
                if os.path.exists(old):
                    if overwrite == 1: os.remove(old)
                        ...
                        if not DIALOG.yesno(ADDONTITLE, "[COLOR %s]Would you like replace the current [COLOR %s]%s[/COLOR] file?" % (COLOR2, COLOR1, item), yeslabel="[B][COLOR dodgerblue]Yes Replace[/COLOR][/B]", nolabel="[B][COLOR red]No Skip[/COLOR][/B]"): continue
                        else: os.remove(old)
        ...
        if os.path.exists(super):
            ...
            if os.path.exists(old):
                cont = DIALOG.yesno(ADDONTITLE, "[COLOR %s]Would you like replace the current [COLOR %s]Super Favourites[/COLOR] addon_data folder with the new one?" % (COLOR2, COLOR1), yeslabel="[B][COLOR dodgerblue]Yes Replace[/COLOR][/B]", nolabel="[B][COLOR red]No Skip[/COLOR][/B]")
            else: cont = 1
            if cont == 1:
                wiz.cleanHouse(old)
...
def freshStart(install=None, over=False):
    ...
    elif install: yes_pressed=DIALOG.yesno(ADDONTITLE, "[COLOR %s]Kodi se restaurara a su configuracion inicial" % COLOR2, "eliminando todos los archivos antes de", "Instalar la Build Base [COLOR %s]%s[/COLOR]" % (COLOR1, install), nolabel='[B][COLOR red]Cancelar[/COLOR][/B]', yeslabel='[B][COLOR dodgerblue]Continuar[/COLOR][/B]')
...
####THANKS GUYS @ NaN #######
def clearCache():
    if DIALOG.yesno(ADDONTITLE, '[COLOR %s]Quieres borrar el cache?[/COLOR]' % COLOR2, nolabel='[B][COLOR red]Cancelar[/COLOR][/B]', yeslabel='[B][COLOR dodgerblue]Borrar Cache[/COLOR][/B]'):
        wiz.clearCache()

def clearArchive():
    if DIALOG.yesno(ADDONTITLE, '[COLOR %s]Quieres borrar la carpeta \'Archive_Cache\'?[/COLOR]' % COLOR2, nolabel='[B][COLOR red]Cancelar[/COLOR][/B]', yeslabel='[B][COLOR dodgerblue]Borrar[/COLOR][/B]'):
        wiz.clearArchive()

def totalClean():
    if DIALOG.yesno(ADDONTITLE, '[COLOR %s]Quieres borrar el cache, los paquetes y thumbnails?[/COLOR]' % COLOR2, nolabel='[B][COLOR red]Cancelar[/COLOR][/B]',yeslabel='[B][COLOR dodgerblue]Borrar todo[/COLOR][/B]'):
        wiz.clearCache()
...
def clearThumb(type=None):
    ...
    else: choice = DIALOG.yesno(ADDONTITLE, '[COLOR %s]Quieres borrar el archivo %s y las carpetas de thumbnails?' % (COLOR2, latest), "Se recargaran en el proximo inicio[/COLOR]", nolabel='[B][COLOR red]No borrar[/COLOR][/B]', yeslabel='[B][COLOR dodgerblue]Borrar[/COLOR][/B]')
    if choice == 1:
