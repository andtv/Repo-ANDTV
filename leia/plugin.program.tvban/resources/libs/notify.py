...
def autoConfig(msg='', TxtColor='0xFFFFFFFF', Font='font12', BorderWidth=10):
	class MyWindow(xbmcgui.WindowDialog):
		...
		def __init__(self,msg='',L=0,T=0,W=1280,H=720,TxtColor='0xFFFFFFFF',Font='font12',BorderWidth=10):
			...
			header = '[COLOR %s]Advanced Settings Configuracion[/COLOR]' % (COLOR2)
			...
			current1 = '[COLOR %s]Presente:[/COLOR] [COLOR %s]%s MB[/COLOR]' % (COLOR1, COLOR2, self.currentVideo)
			...
			header = '[COLOR %s]CURL Tiempo de espera / CURL baja velocidad[/COLOR]' % (COLOR2)
			...
			self.Support3=xbmcgui.ControlTextBox(rightside+int(BorderWidth*3.5), firstrow+30+BorderWidth, (W/2)-(BorderWidth*4), 150, font='font10', textColor=TxtColor)
			self.addControl(self.Support3)
			self.Support3.setText(msg3)
			...
			current2 = '[COLOR %s]Presente:[/COLOR] [COLOR %s]%ss[/COLOR]' % (COLOR1, COLOR2, self.currentCurl)
			...
			self.Support3=xbmcgui.ControlTextBox(leftside+int(BorderWidth*1.5), secondrow+30+BorderWidth, (W/2)-(BorderWidth*4), 150, font='font10', textColor=TxtColor)
			self.addControl(self.Support3)
			self.Support3.setText(msg3)
			...
			current3 = '[COLOR %s]presente:[/COLOR] [COLOR %s]%s[/COLOR]' % (COLOR1, COLOR2, self.currentRead)
			...
			self.Header4=xbmcgui.ControlLabel(rightside+BorderWidth, secondrow+5, (W/2)-(BorderWidth*2), 20, header, font='font10', textColor=TxtColor, alignment=0x00000002)
			self.addControl(self.Header4)
			...
			self.Support4=xbmcgui.ControlTextBox(rightside+int(BorderWidth*3.5), secondrow+30+BorderWidth, (W/2)-(BorderWidth*4), 110, font='font10', textColor=TxtColor)
			self.addControl(self.Support4)
			self.Support4.setText(msg4)
			B1 = secondrow+130+BorderWidth; B2 = B1+30; B3 = B2+30; B4 = B3+30;
			self.Button0 = xbmcgui.ControlRadioButton(rightside+(BorderWidth*3), B1, (W/2)-(BorderWidth*4), 30, '0: Buffer todos los sistemas de archivos de internet', font='font12', focusTexture=radiobgfocus, noFocusTexture=radiobgnofocus, focusOnTexture=radiofocus, noFocusOnTexture=radiofocus, focusOffTexture=radionofocus, noFocusOffTexture=radionofocus)
			self.Button1 = xbmcgui.ControlRadioButton(rightside+(BorderWidth*3), B2, (W/2)-(BorderWidth*4), 30, '1: Buffer todos los sistemas de archivos', font='font12', focusTexture=radiobgfocus, noFocusTexture=radiobgnofocus, focusOnTexture=radiofocus, noFocusOnTexture=radiofocus, focusOffTexture=radionofocus, noFocusOffTexture=radionofocus)
			self.Button2 = xbmcgui.ControlRadioButton(rightside+(BorderWidth*3), B3, (W/2)-(BorderWidth*4), 30, '2: Solo almacenar en buffer los verdaderos sistemas de archivos de Internet', font='font12', focusTexture=radiobgfocus, noFocusTexture=radiobgnofocus, focusOnTexture=radiofocus, noFocusOnTexture=radiofocus, focusOffTexture=radionofocus, noFocusOffTexture=radionofocus)
			self.Button3 = xbmcgui.ControlRadioButton(rightside+(BorderWidth*3), B4, (W/2)-(BorderWidth*4), 30, '3: Sin configuracion del Buffer', font='font10', focusTexture=radiobgfocus, noFocusTexture=radiobgnofocus, focusOnTexture=radiofocus, noFocusOnTexture=radiofocus, focusOffTexture=radionofocus, noFocusOffTexture=radionofocus)
		...
		def doWrite(self):
			...
			if os.path.exists(ADVANCED):
				choice = DIALOG.yesno(ADDONTITLE, "[COLOR %s]Actualmente hay un activo [COLOR %s]AdvancedSettings.xml[/COLOR], te gustaria eliminarlo y continuar?[/COLOR]" % (COLOR2, COLOR1), yeslabel="[B][COLOR green]SI[/COLOR][/B]", nolabel="[B][COLOR red]NO[/COLOR][/B]")
				if choice == 0: return
...
def apkInstaller(apk):
	...
	xbmc.executebuiltin('Skin.SetString(apkinstaller, Ahora %s se esta descargando [CR] Click para continuar!)' % apk)
...
def firstRun():
	class MyWindow(xbmcgui.WindowXMLDialog):
		def __init__(self, *args, **kwargs):
			...
			self.msg   = "Actualmente no hay Build %s instalado.\n\nSelecciona 'Wizard Menu' para instalar la Build TVBAN o 'Ignorar' para no volver a ver este mensaje.\nSi ya tienes una version anterior de %s pulsa ignorar e instala un parche." % (ADDONTITLE, ADDONTITLE)		
		...
		def doBuildMenu(self):
			wiz.log("[Check Updates] [Seleccionado: Abrir menu del Wizard] [Siguiente verificacion: %s]" % str(NEXTCHECK), xbmc.LOGNOTICE)
		...
		def doIgnore(self):
			self.close()
			wiz.log("[First Run] [Seleccionado: Ignorar menu del Wizard] [Siguiente verificacion: %s]" % str(NEXTCHECK), xbmc.LOGNOTICE)
			wiz.setS('lastbuildcheck', str(NEXTCHECK))
...
def notification(msg='', test=False):
	class MyWindow(xbmcgui.WindowXMLDialog):
		...
		def doRemindMeLater(self):
			...
			wiz.log("[Notification] NotifyID %s Mas Tarde" % wiz.getS('noteid'), xbmc.LOGNOTICE)
			self.close()

		def doDismiss(self):
			if not test == True:
				wiz.setS("notedismiss","true")
			wiz.log("[Notification] NotifyID %s Nunca" % wiz.getS('noteid'), xbmc.LOGNOTICE)
			self.close()
...
def updateWindow(name='Testing Window', current='1.0', new='1.1', icon=ICON, fanart=FANART):
	class MyWindow(xbmcgui.WindowXMLDialog):
		...
		def doFreshInstall(self):
			wiz.log("[Updates] [Version: %s] [Version Actual %s] [Selecciona: Fresh Install]" % (BUILDVERSION, LATESTVERSION), xbmc.LOGNOTICE)
			wiz.log("[Updates] [Siquiente chequeo: %s]" % str(NEXTCHECK), xbmc.LOGNOTICE)
			wiz.setS('lastbuildcheck', str(NEXTCHECK))
			self.close()
		...
		def doNormalInstall(self):
			wiz.log("[Updates] [Version: %s] [Version Actual: %s] [Selecciona: Normal]" % (BUILDVERSION, LATESTVERSION), xbmc.LOGNOTICE)
			wiz.log("[Updates] [Siquiente chequeo: %s]" % str(NEXTCHECK), xbmc.LOGNOTICE)
			wiz.setS('lastbuildcheck', str(NEXTCHECK))
			self.close()
		...
		def doIgnore(self):
			wiz.log("[Updates] [Version: %s] [Version Actual: %s] [Selecciona: Ignorar en 3 Dias]" % (BUILDVERSION, LATESTVERSION), xbmc.LOGNOTICE)
			wiz.log("[Updates] [Siquiente chequeo: %s]" % str(THREEDAYS), xbmc.LOGNOTICE)
