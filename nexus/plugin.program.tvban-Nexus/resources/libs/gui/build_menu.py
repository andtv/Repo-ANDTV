    ...
    def _list_all(self, match, kodiv=None):
        ...
        for name, version, url, gui, kodi, theme, icon, fanart, adult, description in match:
            ...
            if not kodiv or kodiv == int(float(kodi)):
                ...
                if float(kodi) == 21.0:
                    directory.add_dir('[COLOR aliceblue][/COLOR] [B][COLOR azure]•[/COLOR][COLOR yellowgreen] {1} - v{2}[/COLOR][/B]'.format(name, version), {'mode': 'viewbuild', 'name': name}, description=description, fanart=fanart, icon=icon, menu=menu, themeit=CONFIG.THEME2)
