...
host = 'https://www11.series24.cc/'


# ~ por si viene de enlaces guardados
ant_hosts = ['https://www.series24.cc/', 'https://www1.series24.cc/', 'https://ww3.series24.cc/',
            'https://ww2.series24.cc/']


domain = config.get_setting('dominio', 'series24', default='')

if domain:
    if domain == host: config.set_setting('dominio', '', 'series24')
    elif domain in str(ant_hosts): config.set_setting('dominio', '', 'series24')
    else: host = domain
...
def item_configurar_proxies(item):
    ...
    if config.get_setting('channel_series24_proxies', default=''):
...
def do_downloadpage(url, post=None, headers=None, raise_weberror=True):
    ...
    if not url.startswith(host):
        ...
        data = httptools.downloadpage_proxy('series24', url, post=post, headers=headers, raise_weberror=raise_weberror).data
...
def acciones(item):
    ...
    domain_memo = config.get_setting('dominio', 'series24', default='')
...
def mainlist_series(item):
    ...
    itemlist.append(item.clone( title = 'Últimos episodios', action = 'last_epis', url = host + 'ver-serie-online/', search_type = 'tvshow' ))
        ...
        if tvdb_id:
            ...
            if sum_parts >= 1000:
                if platformtools.dialog_yesno(item.contentSerieName.replace('&#038;', '&').replace('&#8217;', "'"), '¿ Hay [COLOR yellow][B]' + str(sum_parts) + '[/B][/COLOR] elementos disponibles, desea cargarlos en bloques de [COLOR cyan][B]500[/B][/COLOR] elementos ?'):
                    platformtools.dialog_notification('Series24', '[COLOR cyan]Cargando 500 elementos[/COLOR]')
                    item.perpage = 500

            elif sum_parts >= 500:
                if platformtools.dialog_yesno(item.contentSerieName.replace('&#038;', '&').replace('&#8217;', "'"), '¿ Hay [COLOR yellow][B]' + str(sum_parts) + '[/B][/COLOR] elementos disponibles, desea cargarlos en bloques de [COLOR cyan][B]250[/B][/COLOR] elementos ?'):
                    platformtools.dialog_notification('Series24', '[COLOR cyan]Cargando 250 elementos[/COLOR]')
                    item.perpage = 250

            elif sum_parts >= 250:
                if platformtools.dialog_yesno(item.contentSerieName.replace('&#038;', '&').replace('&#8217;', "'"), '¿ Hay [COLOR yellow][B]' + str(sum_parts) + '[/B][/COLOR] elementos disponibles, desea cargarlos en bloques de [COLOR cyan][B]125[/B][/COLOR] elementos ?'):
                    platformtools.dialog_notification('Series24', '[COLOR cyan]Cargando 125 elementos[/COLOR]')
                    item.perpage = 125

            elif sum_parts >= 125:
                if platformtools.dialog_yesno(item.contentSerieName.replace('&#038;', '&').replace('&#8217;', "'"), '¿ Hay [COLOR yellow][B]' + str(sum_parts) + '[/B][/COLOR] elementos disponibles, desea cargarlos en bloques de [COLOR cyan][B]75[/B][/COLOR] elementos ?'):
                    platformtools.dialog_notification('Series24', '[COLOR cyan]Cargando 75 elementos[/COLOR]')
                    item.perpage = 75
